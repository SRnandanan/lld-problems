...
public class StackOverFlowDemo {
    public static void main(String[] args) {
        ...
        Question javaQuestion = system.addQuestion(alice, "What is polymorphism in Java?",
                "Can someone explain polymorphism in Java with an example?",
                Arrays.asList("java", "oop"));

        // Bob answers Alice's question
        Answer bobAnswer = system.addAnswer(bob, javaQuestion,
        ...
        system.voteQuestion(charlie, 1, javaQuestion);  // Upvote
        system.voteAnswer(charlie, 1, bobAnswer);  // Upvote

        // Alice accepts Bob's answer
        system.acceptAnswer(bobAnswer);

        // Bob asks another question
        Question pythonQuestion = system.addQuestion(bob, "How to use list comprehensions in Python?",
        ...
        Answer aliceAnswer = system.addAnswer(alice, pythonQuestion,
                "List comprehensions in Python provide a concise way to create lists...");

        // Charlie votes on Bob's question and Alice's answer
        system.voteQuestion(charlie, 1, pythonQuestion);  // Upvote
        system.voteAnswer(charlie, 1, aliceAnswer);  // Upvote
        ...
        List<Question> searchResults = system.searchForQuestion("java");
        for (Question q : searchResults) {
            System.out.println(q.getTitle());
        }

        System.out.println("\nSearch Results for 'python':");
        searchResults = system.searchForQuestion("python");
        for (Question q : searchResults) {
            System.out.println(q.getTitle());
        }
